`timescale 1ns/1ps
// qmul_bank_tagged.sv — shared_qmul3 bank with a parallel tag pipeline.
// QCOL = LAT+1 = 4: one operand-register cycle + 3 bank cycles.
// p0/p1/p2 are valid when res_valid is high, labelled by res_tag ({slot,dest}).
module qmul_bank_tagged #(
    parameter int TAG_W = 8,
    parameter int QCOL  = 4
)(
    input  logic               clk,
    input  logic               iss_valid,
    input  logic [TAG_W-1:0]   iss_tag,
    input  logic signed [31:0] a0, b0, a1, b1, a2, b2,
    output logic               res_valid,
    output logic [TAG_W-1:0]   res_tag,
    output logic signed [31:0] p0, p1, p2
);
    // Operand register — the +1 in QCOL = LAT+1.
    logic signed [31:0] a0_r, b0_r, a1_r, b1_r, a2_r, b2_r;
    always_ff @(posedge clk) begin
        a0_r <= a0; b0_r <= b0;
        a1_r <= a1; b1_r <= b1;
        a2_r <= a2; b2_r <= b2;
    end

    shared_qmul3 #(.LAT(3)) u_bank (
        .clk (clk),
        .a0  (a0_r), .b0 (b0_r),
        .a1  (a1_r), .b1 (b1_r),
        .a2  (a2_r), .b2 (b2_r),
        .p0  (p0),   .p1 (p1),   .p2 (p2)
    );

    // Tag/valid shift register, depth QCOL, aligned with the operand register.
    logic             v_pipe [0:QCOL-1];
    logic [TAG_W-1:0] t_pipe [0:QCOL-1];

    // Zero-init prevents X-propagation through res_valid in simulation.
    initial begin
        for (int i = 0; i < QCOL; i++) begin
            v_pipe[i] = 1'b0;
            t_pipe[i] = '0;
        end
    end

    always_ff @(posedge clk) begin
        v_pipe[0] <= iss_valid;
        t_pipe[0] <= iss_tag;
        for (int i = 1; i < QCOL; i++) begin
            v_pipe[i] <= v_pipe[i-1];
            t_pipe[i] <= t_pipe[i-1];
        end
    end

    assign res_valid = v_pipe[QCOL-1];
    assign res_tag   = t_pipe[QCOL-1];

endmodule
