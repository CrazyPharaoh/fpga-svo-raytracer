`timescale 1ns/1ps
// pixel_reorder.sv — in-order pixel reorder buffer for the multi-ray core.
//
// Decouples ray completion from AXIS emission. A slot frees the instant its ray
// completes: its colour is written into a position-indexed circular buffer and the
// slot relaunches immediately, up to REORDER_DEPTH pixels ahead of the emit pointer.
// A separate emit pointer walks the buffer in raster order and streams to AXIS with
// correct framing (tlast at line end, tuser at frame start). Fast rays run ahead of
// a slow one instead of idling.
//
// REORDER_DEPTH (power of 2) is the run-ahead window: a slot may launch at most DEPTH
// pixels ahead of emit_idx, beyond which launches stall (buffer full).
//
// The core launches on launch_valid and reports completion on done_valid; this module
// owns AXIS + busy/frame_done.
